...
if __name__ == '__main__':
    from configs.drone.v19 import *
else:
    from configs.drone.v5 import *
...
def train_barrier(bnn, swa_bnn, optimizer, buf_traj, buf_free, buf_danger, buf_agent, buf_obstacle, n_iter=10):
    ...
    def compute_bloss(bnn, data):
        ...
        bloss = (bloss1 + bloss2).sum() / (data['next_free'].sum()+data['next_danger'].sum())
        return bloss
    ...
    def compute_dloss(bnn, data, next_data):
        ...
        if not ALL_LIE:
            deriv = next_value+(-value+DISCOUNT*value)
            dloss = ((-deriv+GAMMA).relu())*data['next_free']*next_data['next_free']
            dloss = dloss.sum() / (1e-9 + (data['next_free']*next_data['next_free']).sum())
        else:
            deriv = next_value+(-value+DISCOUNT*value)
            dloss = ((-deriv+GAMMA).relu())
            dloss = dloss.mean()
...
#                     if (prev_free & next_danger & (~meet_obstacle) & (~meet_obstacle)).any():
#                         data = info.clone()
#                         data['prev_free'] = torch.FloatTensor([False]*env.num_agents)
#                         data['next_free'] = torch.FloatTensor([False]*env.num_agents)
#                         data['next_danger'] = torch.FloatTensor(prev_free & next_danger & (~meet_obstacle) & (~meet_obstacle))
...
#                         cbuf_dynamic_free.store(data)    
                    ...
                    # data['prev_free'] = torch.FloatTensor([False]*env.num_agents)
                    # data['next_danger'] = torch.FloatTensor([False]*env.num_agents)
                    if ((data['next_danger'] + data['next_free'])>=1).any():
                        cbuf_dynamic_free.store(data)
        ...
        if (epoch_i % N_TRAJ_PER_UPDATE == (N_TRAJ_PER_UPDATE-1)) and (epoch_i > N_WARMUP):
            ...
            if len(cbuf_agent.obs_buf) > N_CBUF_AGENT:
                shuffle(cbuf_agent.obs_buf)
                cbuf_agent.obs_buf = cbuf_agent.obs_buf[:N_CBUF_AGENT]

            if len(cbuf_obstacle.obs_buf) > N_CBUF_OBSTACLE:
                shuffle(cbuf_obstacle.obs_buf)
                cbuf_obstacle.obs_buf = cbuf_obstacle.obs_buf[:N_CBUF_OBSTACLE]
