...
if __name__ == '__main__':
    Env = DubinsCarEnv
    env = Env(num_agents=3, PROB=(0,0.1), SIZE=(8,8))
    ...
    #     env.world.agents = env.world.sample_agents(env.num_agents, prob=0.0)
        ...
        with open('1model_'+Env.__name__+'.txt', 'a+') as f:
            f.write(pbar.desc+'\t'+str(epoch_i)+'\t'+'collided: {0:.2f}'.format(collided.mean())+' running rate: {0:.6f}, {1:6f}'.format(running_unsafe_rate, np.mean(uncollide_rates))+' no feasible: {0:d}'.format(no_feasible)+', buf size {0:d} {1:d} {2:d}'.format(len(bbuf.obs_buf), len(cbuf.obs_buf), visit_time)+'\n')

        if (epoch_i % (N_TRAJ//25) == ((N_TRAJ//25)-1)):
