...
def train_lya(lnn, optimizer, buf, pbar, n_iter=10):
    
    # Set up function for computing value loss
    def compute_loss(lnn, data, next_data):
        value = lnn(x=data['toward'].edge_attr, action=data['action'])
        next_value = lnn(x=next_data['toward'].edge_attr, action=data['action'])
        
        gloss1 = (value).abs()*data['next_goal'] / (1e-9 + (data['next_goal']).sum())
        gloss2 = (next_value).abs()*next_data['next_goal'] / (1e-9 + (next_data['next_goal']).sum())
        gloss = gloss1.sum() + gloss2.sum()
        
        deriv = next_value-value
        dloss = (deriv+1e-2)**2  #.relu()
        dloss = dloss.mean()
    ...
    for i in range(n_iter):
        for data, next_data in loader:
            ...
            optimizer.zero_grad()
            gloss, dloss = compute_loss(lnn, data, next_data)
            loss = gloss + dloss
            loss.backward()            
            optimizer.step()
            optimizer.zero_grad()
            pbar.set_description("iter #%d, gloss %.6f, dloss %.6f" % (i, gloss, dloss))
...
class GlobalReplayBuffer:
    ...
    def relabel(self, space_dim, goal_threshold, clip=False):
        # 1. get the final state 
        # TODO: change this to multi-agent level checking
        final_state = self.obs_buf[-1]
        if final_state['next_goal'].any():
            return
        
        # 2. relabel the next_goal thing
        subgoal = self.obs_buf[np.random.randint(low=1, high=len(self.obs_buf))]['agent'].pos
        for idx, obs in enumerate(self.obs_buf[:-1]):
            next_state = self.obs_buf[idx]['agent'].pos
            if clip:
                obs['goal'].pos[:,:space_dim] = torch.clamp(subgoal[:,:space_dim]-obs['agent'].pos[:,:space_dim], -6, 6)+obs['agent'].pos[:,:space_dim]
            else:
                obs['goal'].pos[:,:space_dim] = subgoal[:,:space_dim]
            obs['next_goal'] = ((next_state[:, :space_dim]-obs['goal'].pos[:,:space_dim]).norm(dim=-1) < goal_threshold).float()
            obs['goal', 'toward', 'agent'].edge_attr = obs['goal'].pos[:,:space_dim]-obs['agent'].pos[:,:space_dim]
            obs['goal', 'toward', 'agent'].edge_attr = torch.cat((obs['agent'].pos[:,space_dim:], obs['goal', 'toward', 'agent'].edge_attr), dim=-1)
        
        # 3. clip the trajectory to make the final (state, action) pair to be the first pair that has next_goal=True
        for idx, obs in enumerate(self.obs_buf):
            if obs['next_goal'].any():
                break
        self.obs_buf = self.obs_buf[:(idx+1)]
...
def iter_action(bnn, o, a, threshold, max_iter=30):
    # size of a: (n_candidates, action_dim)
...
if __name__=='__main__':
    ...
    #     env.world.agents = env.world.sample_agents(env.num_agents, prob=0.0)
        ...
        bnn.train()
        train_lya(bnn, boptimizer, bbuf_gather, writer, pbar=pbar, n_iter=N_ITER)
