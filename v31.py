...
if __name__ == '__main__':
    ...
    for _ in tqdm(range(N_DATASET)):
        env = Env(num_agents=NUM_AGENTS, PROB=(0.,OBSTACLE_DENSITY), SIZE=(MAP_SIZE,MAP_SIZE), agent_top_k=AGENT_TOP_K, obstacle_top_k=OBSTACLE_TOP_K)
        # while True:
        #     env = Env(num_agents=NUM_AGENTS, PROB=(0.,OBSTACLE_DENSITY), SIZE=(MAP_SIZE,MAP_SIZE), agent_top_k=AGENT_TOP_K, obstacle_top_k=OBSTACLE_TOP_K)
    ...
    for epoch_i in range(N_TRAJ):
        ...
        if CYCLIC:
            explore_eps = (MAX_EXPLORE_EPS-(MAX_EXPLORE_EPS-MIN_EXPLORE_EPS)*(epoch_i % 100)/100.)  # if ((epoch_i % 200) < 100) else 0.
        else:
            explore_eps = np.clip(MAX_EXPLORE_EPS * (DECAY_EXPLORE_RATE ** ((epoch_i // N_VALID))), MIN_EXPLORE_EPS, MAX_EXPLORE_EPS)

        nominal_eps = DECAY_NOMINAL_RATE ** (1e-5 + (epoch_i // N_VALID))
        ...
        total_trans=0; n_danger=0; threshold=THRESHOLD; no_feasible=0; collided=np.zeros(env.num_agents).astype(bool); volumes=[]; n_relabels=0
        ...
        while True:
            ...
            volumes.append((bvalues>threshold).mean())
