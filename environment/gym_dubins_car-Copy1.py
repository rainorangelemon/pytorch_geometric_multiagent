...
class DubinsCarEnv:
    ...
    # Initialize env
    def __init__(self, num_agents=1, observation_size=6, top_k=None, connected=False, SIZE=(10,40), PROB=(0,.5)):
        """
        Args:
    ...
    def _get_obs(self, loop=False, clip=False):
        data = HeteroData()
        data['agent'].x = torch.FloatTensor(self.world.agents)
        agent_to_agent = data['agent', 'a_near_a', 'agent'].edge_index = knn_graph(data['agent'].x[:,:self.space_dim], self.top_k, loop=loop)
    ...
    def _render(self,close=False,screen_width=800,screen_height=800,action_probs=None):
        env = self
