...
from environment.gym_dynamic_dubins import DynamicDubinsEnv
...
from configs.dynamic_dubins.v6 import *
...
def choose_action(bnn, env, explore_eps, nominal_eps, spatial_prop, thresholds, n_action=None, decompose=None):
    ...
    for agent_id, a_refine, bvalue, dist, threshold in zip(np.arange(env.num_agents), a_refines, bvalues, dists, thresholds):
        ...
        if DANGER_EXPLORE and (not feasible_current) and (explore_eps!=0):
            a[agent_id] = a[agent_id] + (explore_eps!=0)*EXPLORE_NOISE*np.random.randn(*(a[agent_id].shape))
...
def infer(env, bnn, threshold=None, max_episode_length=256, 
          n_action=None,
          verbose=False, seed=0, stop_at_collision=False, get_actions=False,
          get_admissibles=False,
    ...
    admissibles.append([True]*len(feasibles))
    if (not get_actions) and (not get_admissibles):
        return collided, done, paths
    elif (not get_admissibles):
        return collided, done, paths, actions
    elif (not get_actions):
        return collided, done, paths, admissibles
    else:
        return collided, done, paths, actions, admissibles
