...
if __name__ == '__main__':
    from configs.drone.v20 import *
else:
    from configs.dubins.v125 import *
...
if __name__ == '__main__':
    ...
    for epoch_i in range(N_TRAJ):
        ...
        if len(cbuf_dynamic_free.obs_buf) > N_DYNAMIC_BUFFER:
            shuffle(cbuf_dynamic_free.obs_buf)
            cbuf_dynamic_free.obs_buf = cbuf_dynamic_free.obs_buf[:N_DYNAMIC_BUFFER]

        if len(cbuf_dynamic_danger.obs_buf) > N_DYNAMIC_BUFFER:
            shuffle(cbuf_dynamic_danger.obs_buf)
            cbuf_dynamic_danger.obs_buf = cbuf_dynamic_danger.obs_buf[:N_DYNAMIC_BUFFER]            
