...
def train_barrier(bnn, swa_bnn, optimizer, buf_traj, buf_free, buf_danger, buf_agent, buf_obstacle, n_iter=10):
    ...
    def compute_bloss(bnn, data, restrict=True):
        value = bnn(data)
        
        bloss1 = ((1e-1-value).relu())*data['next_free']
        bloss2 = ((1e-1+value).relu())*data['next_danger']
        if restrict:
            bloss1 = bloss1.clip(0, 5e-2)
            bloss2 = bloss2.clip(0, 5e-2)
        bloss1 = bloss1 / (1e-9 + (data['next_free']).sum())
        bloss2 = bloss2 / (1e-9 + (data['next_danger'].sum()))
        bloss = bloss1.sum() + bloss2.sum()
        return bloss

    def compute_dloss(bnn, data, next_data, restrict=True):
        value = bnn(data)
        ...
        if restrict:
            dloss = dloss.clip(0, 5e-2)
        if not ALL_LIE:
            dloss = dloss*data['next_free']*next_data['next_free']
    ...
    for i in range(n_iter):
        if len(buf_traj.dataset):
            ...
            dloss = compute_dloss(bnn, data, next_data, restrict=False)
        else:
            dloss = 0
        ...
        if len(buf_free):
            ...
            optimizer.zero_grad()
            data = data.to(device)
            bloss_f = compute_bloss(bnn, data, restrict=False)
        else:
            bloss_f = 0
        ...
        if len(buf_danger):
            ...
            optimizer.zero_grad()
            data = data.to(device)
            bloss_d = compute_bloss(bnn, data, restrict=False)
        else:
            bloss_d = 0        
        ...
        if len(buf_agent):
            ...
            optimizer.zero_grad()
            data = data.to(device)
            closs_a = compute_bloss(bnn, data, restrict=False)   
        else:
            closs_a = 0
        ...
        if len(buf_obstacle):
            ...
            optimizer.zero_grad()
            data = data.to(device)
            closs_o = compute_bloss(bnn, data, restrict=False)
        else:
            closs_o = 0
        
        (bloss_f + bloss_d + dloss + closs_a + closs_o).backward()
        if CLIP_NORM:
            torch.nn.utils.clip_grad_norm_(bnn.parameters(), CLIP_NORM_VALUE)
        optimizer.step()
        optimizer.zero_grad()
...
class GlobalReplayBuffer:
    ...
    def next_data(self):
        if SHARE_SAMPLE_ACROSS_UPDATE:
            ...
            return_data = Batch.from_data_list(self.obs_buf[self.pointer:(self.pointer+self.batch)])
            if (self.pointer + 2*self.batch) >= (len(self.obs_buf)):
                self.pointer = 0
                shuffle(self.obs_buf)
            else:
                self.pointer = self.pointer + self.batch
            return return_data            
...
class TrajectoryReplayBuffer:
    ...
    def next_data(self):
        if SHARE_SAMPLE_ACROSS_UPDATE:
            ...
            return_data = collate(self.dataset[self.pointer:(self.pointer+self.batch)])
            if (self.pointer + 2*self.batch) >= (len(self.dataset)):
                idxs = np.random.permutation(len(self.dataset))
                self.dataset.data = [self.dataset.data[idx] for idx in idxs]
                self.dataset.next_data = [self.dataset.next_data[idx] for idx in idxs]
                self.pointer = 0
            else:
                self.pointer = self.pointer + self.batch
            return return_data
...
class GatherReplayBuffer(Dataset):
    ...
    def next_data(self):
        if SHARE_SAMPLE_ACROSS_UPDATE:
            ...
            return_data = Batch.from_data_list(self.dataset[self.pointer:(self.pointer+self.batch)])
            if (self.pointer + 2*self.batch) >= (len(self.dataset)):
                self.pointer = 0
...
def infer(env, bnn, threshold=None, max_episode_length=256, 
    ...
    if (need_gif is not None) and (np.any(collided)):
...
if __name__ == '__main__':
    ...
    wandb.init(
        ...
        config={
            ...
            "shuffle_when_exceed": SHUFFLE_WHEN_EXCEED,
    ...
    for epoch_i in range(N_TRAJ):
        ...
        if len(bbuf_gather.dataset) > N_BUFFER:
            if SHUFFLE_WHEN_EXCEED:
                shuffle(bbuf_gather.dataset)
            bbuf_gather.dataset = bbuf_gather.dataset[-N_BUFFER:]

        if SHUFFLE_WHEN_TRAIN:
            shuffle(bbuf_gather.dataset)
            shuffle(cbuf_agent.obs_buf)
            shuffle(cbuf_obstacle.obs_buf)
            shuffle(cbuf_dynamic_free.obs_buf)
            shuffle(cbuf_dynamic_danger.obs_buf)
            if len(bbuf_traj.dataset):
                idxs = np.random.choice(len(bbuf_traj.dataset), min(N_TRAJ_BUFFER, len(bbuf_traj.dataset)), replace=False)
                bbuf_traj.dataset.data = [bbuf_traj.dataset.data[idx] for idx in idxs]
                bbuf_traj.dataset.next_data = [bbuf_traj.dataset.next_data[idx] for idx in idxs]            
        
        if len(bbuf_traj.dataset) > N_TRAJ_BUFFER:
            if SHUFFLE_WHEN_EXCEED:
                idxs = np.random.choice(len(bbuf_traj.dataset), N_TRAJ_BUFFER, replace=False)
                bbuf_traj.dataset.data = [bbuf_traj.dataset.data[idx] for idx in idxs]
                bbuf_traj.dataset.next_data = [bbuf_traj.dataset.next_data[idx] for idx in idxs]
            else:
                bbuf_traj.dataset.data = bbuf_traj.dataset.data[-N_TRAJ_BUFFER:]
                bbuf_traj.dataset.next_data = bbuf_traj.dataset.next_data[-N_TRAJ_BUFFER:]

        if len(cbuf_agent.obs_buf) > N_CBUF:
            if SHUFFLE_WHEN_EXCEED:
                shuffle(cbuf_agent.obs_buf)
            cbuf_agent.obs_buf = cbuf_agent.obs_buf[-N_CBUF:]

        if len(cbuf_obstacle.obs_buf) > N_CBUF:
            if SHUFFLE_WHEN_EXCEED:            
                shuffle(cbuf_obstacle.obs_buf)
            cbuf_obstacle.obs_buf = cbuf_obstacle.obs_buf[-N_CBUF:]

        if len(cbuf_dynamic_free.obs_buf) > N_DYNAMIC_BUFFER_FREE:
            if SHUFFLE_WHEN_EXCEED:
                shuffle(cbuf_dynamic_free.obs_buf)
            cbuf_dynamic_free.obs_buf = cbuf_dynamic_free.obs_buf[-N_DYNAMIC_BUFFER_FREE:]

        if len(cbuf_dynamic_danger.obs_buf) > N_DYNAMIC_BUFFER_DANGER:
            if SHUFFLE_WHEN_EXCEED:
                shuffle(cbuf_dynamic_danger.obs_buf)
