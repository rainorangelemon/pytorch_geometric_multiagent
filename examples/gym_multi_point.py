...
#         return moved
    ...
    def get_status(self):
        ...
        for agent_id, agent in enumerate(self.agents):
            s = ''
            if (len(self.obstacles)!=0) and cdist(self.obstacles, np.array([agent])).min() < OBSTACLE_DISTANCE_THRESHOLD:
                s = s+'danger_obstacle'
            if (len(agents) > 1) and (distance_nearest[agent_id] < AGENT_DISTANCE_THRESHOLD):
                s = s+'danger_agent'
            # if (len(agents) > 1) and (distance_nearest[agent_id] > 0.7):
            #     s = s+'safe'
            if np.linalg.norm(np.array(self.agents[agent_id])-np.array(self.agent_goals[agent_id])) < GOAL_THRESHOLD:
                s = s+'done'
            if 'danger' not in s:
                s = s+'free'
            status.append(s)
    ...
    def done(self):
        ...
        for i in range(len(self.agents)):
            if np.linalg.norm(np.array(self.agents[i])-np.array(self.agent_goals[i])) < GOAL_THRESHOLD:
                numComplete += 1
...
class MultiPointEnv():
    ...
    def _get_obs(self, mode=None, loop=False, clip=False):
        ...
        data['agent'].x = torch.FloatTensor([[1,0,0]]*self.num_agents)
        if len(self.world.obstacles) != 0:
            data['obstacle'].x = torch.FloatTensor([[0,1,0]]*len(self.world.obstacles))
        else:
            data['obstacle'].x = torch.FloatTensor([[0,1,0]])
        data['goal'].x = torch.FloatTensor([[0,0,1]]*self.num_agents)
