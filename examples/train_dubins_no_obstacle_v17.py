...
v17. change lr to 1e-4
'''


Env = DubinsCarEnv
name_dict = generate_default_model_name(Env)
TXT_NAME = '1model_'+Env.__name__+'_v17.txt'
BMODEL_PATH = name_dict['db'].replace('.pt', '_v17.pt')
...
def infer(env, bnn, verbose=False, seed=0):
    ...
    while True:
        ...
        dists = env.potential_field(a_refines, K1=1e-1, K2=-1e-2, ignore_agent=True)
...
if __name__ == '__main__':
    ...
    for epoch_i in pbar:
        ...
        if CYCLIC:
            ...
            explore_eps = np.clip(1. - 3 * (epoch_i // N_VALID) / (N_TRAJ // N_VALID), 0, 0.5)
    ...
    #     env.world.agent_goals = np.array([[half_size-(half_size-0.5)*np.cos(a), half_size-(half_size-0.5)*np.sin(a)]+[0.]*(Env.state_dim-2) for a in np.linspace(0, 2*np.pi, NUM_AGENTS, endpoint=False)])
        ...
        if (epoch_i % (N_VALID) == (N_VALID-1)):
            ...
            torch.save(bnn.state_dict(), BMODEL_PATH.replace('.pt', '{0:d}.pt'.format(epoch_i)))
            
            # bscheduler.step(np.mean(uncollide_rates))
