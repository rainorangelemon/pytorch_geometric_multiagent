...
def create_network():
    ...
    bnn.to(device)
    bnn.load_state_dict(torch.load('model_gnn/dbgnn_DubinsCarEnv_v24.pt', map_location=device))
    return bnn
...
def train_barrier(bnn, optimizer, buf, pbar, n_iter=10):
    ...
    for i in range(n_iter):
        for data, next_data in loader:
            ...
            optimizer.step()
            optimizer.zero_grad()
            pbar.set_description(version_name+": iter #%d, bloss %.6f, dloss %.6f, closs %.6f" % (i, bloss, dloss, closs))
...
@torch.no_grad()
def infer(env, bnn, threshold=None, max_episode_length=256, verbose=False, seed=0):
...
if __name__ == '__main__':
    ...
    for epoch_i in pbar:
        ...
        if CYCLIC:
            explore_eps = (1.-(epoch_i % 100)/100.) if ((epoch_i % 200) < 100) else 0.
        else:
            explore_eps = np.clip(0.1 - DECAY_EXPLORE_RATE * (epoch_i // N_VALID) / (N_TRAJ // N_VALID), MIN_EXPLORE_EPS, 0.1)

        # if epoch_i > N_WARMUP:
        #     nominal_eps = 0.8 ** ((epoch_i // N_VALID))
