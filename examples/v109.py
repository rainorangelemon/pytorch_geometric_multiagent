...
def infer(env, bnn, threshold=None, max_episode_length=256, 
    ...
    total_trans=0; n_danger=0; no_feasible=0; 
    collided=np.zeros(env.num_agents).astype(bool); thresholds=np.array([threshold]*env.num_agents)
    ...
    while True:
        ...
        next_goal = info['next_goal'].data.cpu().numpy().astype(bool)
