...
def create_network():
    if ENV_CONFIG['PROB'][1] == 0:
        bnn = eval(MODEL)(HIDDEN_SIZE, keys=['agent'], num_layers=1, pos_encode=PE_DIM)
    else:
        bnn = eval(MODEL)(HIDDEN_SIZE, keys=['agent', 'obstacle'], num_layers=1, pos_encode=PE_DIM)
    bnn.to(device)
    return bnn
...
def train_barrier(bnn, swa_bnn, optimizer, buf_traj, buf_free, buf_danger, buf_agent, buf_obstacle, n_iter=10):
    ...
    def compute_dloss(bnn, data, next_data):
        ...
        bloss1 = ((-value).relu())*data['next_free'] / (1e-9 + (data['next_free']).sum())
        bloss2 = ((-value).relu())*next_data['next_free'] / (1e-9 + (next_data['next_free'].sum()))
        bloss = bloss1.sum() + bloss2.sum()         
...
def infer(env, bnn, threshold=None, max_episode_length=256, 
    ...
    arm_ids = deepcopy(env.world.arm_ids)
            
    if need_gif is not None:
        if (not only_show_collide) or (np.any(collided)):
            env = create_env(num_agents=env.num_agents, arm_ids=arm_ids, GUI=True, save_file=need_gif[:-4]+'_'+str(np.any(collided))+'_'+str(done)+need_gif[-4:])
...
if __name__ == '__main__':
    ...
    run.log_code(".", include_fn=lambda path: ("arm_env2.py" in path) or ("gym_ur5.py" in path))
    ...
    for _ in tqdm(range(N_VALID_DATASET)):
        ...
        env = create_env(randomize=True)
        valid_dataset.append(env.world.arm_ids)
    ...
    arm_ids_choices = [[0,1],[0,2],[0,3],[1,2],[1,3],[2,3],[1,0],[2,0],[3,0],[2,1],[3,1],[3,2]]
    ...
    for epoch_i in range(N_TRAJ):
        ...
        env = create_env(arm_ids=arm_ids_choices[epoch_i%len(arm_ids_choices)], randomize=True)
        ...
        while True:
            ...
            if done or (total_trans >= max_episode_length):
                ...
                if (n_danger!=0):
                    if RELABEL:
                        if (epoch_i > N_WARMUP) and (epoch_i>=N_TRAJ_PER_UPDATE):
                ...
                for info in bbuf.obs_buf:
                    ...
                    if (next_danger & (~meet_obstacle) & (~meet_agent)).any():
                        data = info.clone()
                        data['prev_free'] = torch.FloatTensor([False]*env.num_agents)
                        data['next_free'] = torch.FloatTensor([False]*env.num_agents)
                        data['next_danger'] = torch.FloatTensor(next_danger & (~meet_obstacle) & (~meet_agent))
                    ...
                    if (next_free).any():
                        ...
                        data['next_free'] = torch.FloatTensor(next_free)
        ...
        if (epoch_i > N_WARMUP) and (epoch_i % (N_VALID) == (N_VALID-1)):
            ...
            for v_idx, data in enumerate(valid_dataset):
                arm_ids = deepcopy(data)
                env = create_env(num_agents=len(arm_ids), arm_ids=arm_ids, randomize=True)
