...
def train_barrier(bnn, optimizer, buf, pbar, n_iter=10):
    ...
    for i in range(n_iter):
        ...
        if (i % 10 == 0) and (i != 0):
            with torch.no_grad():
                val_losses = []
                for data, next_data in valid_loader:
                    bloss, dloss = compute_loss(bnn, data.to(device), next_data.to(device))
                    val_losses.append(bloss+dloss)

                if USE_SCHEDULER:
                    scheduler.step(torch.tensor(val_losses).mean())        

                wandb.log({"valid_loss": torch.tensor(val_losses).mean(),
...
class GatherReplayBuffer:
    ...
    def construct_dataset(self, valid_ratio=0.2):
        n_valid_buffer = int(len(self.buffers)*0.2)
        prev_o = []
        prev_o.extend([o for b in self.buffers[:-n_valid_buffer] for o in b.obs_buf[:-1]])
        next_o = []
        next_o.extend([o for b in self.buffers[:-n_valid_buffer] for o in b.obs_buf[1:]])
        prev_o_valid = []
        prev_o_valid.extend([o for b in self.buffers[-n_valid_buffer:] for o in b.obs_buf[:-1]])
        next_o_valid = []
        next_o_valid.extend([o for b in self.buffers[-n_valid_buffer:] for o in b.obs_buf[1:]])
        self.dataset = MyDataset(prev_o, next_o)
        self.valid_dataset = MyDataset(prev_o_valid, next_o_valid)
    ...
    def get(self):
        ...
        valid_loader = DataLoader(self.valid_dataset, shuffle=False, batch_size=self.batch, num_workers=8, collate_fn=collate)
...
if __name__ == '__main__':
    ...
    for epoch_i in pbar:
        ...
        if (OUTDATE == 0) and (epoch_i % N_VALID==0):
            cbuf = GlobalReplayBuffer()
        ...
        with open(TXT_NAME, 'a+') as f:
            f.write(pbar.desc+'\t'+str(epoch_i)+'\t'+'collided: {0:.2f}'.format(collided.mean())+' running rate: {0:.6f}, {1:.6f}, {2:.6f}'.format(running_unsafe_rate, np.mean(uncollide_rates), np.mean(success_rates))+' no feasible: {0:d}'.format(no_feasible)+', buf size {0:d} {1:d} {2:d}'.format(len(bbuf.obs_buf), len(cbuf.obs_buf), visit_time)+' lr: {0:.2e}'.format(boptimizer.param_groups[0]['lr'])+'\n')
