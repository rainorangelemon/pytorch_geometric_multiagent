...
class GlobalReplayBuffer:
    ...
    def relabel(self):
        ...
        for idx, obs in list(zip(range(len(self.obs_buf)), self.obs_buf))[::-1]:
            ...
            obs['prev_danger'] = ((obs['prev_danger'] + (obs['next_danger'] * (1-obs['feasible']))) >= 1).float()
...
if __name__ == '__main__':
    ...
    for _ in tqdm(range(N_DATASET)):
        while True:
            ...
            if (np.linalg.norm(env.world.agents - env.world.agent_goals, axis=-1).min() >= 2) and (len(env.world.obstacles)!=0):
                break
    ...
    bscheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(boptimizer, factor=0.5, patience=10, eps=1e-6)  
    ...
    for epoch_i in pbar:
        ...
        while True:
            env = Env(num_agents=NUM_AGENTS, mode='barrier', PROB=(0,OBSTACLE_DENSITY), SIZE=(MAP_SIZE,MAP_SIZE))
            if (np.linalg.norm(env.world.agents[:,:2] - env.world.agent_goals[:,:2], axis=-1).min() >= 2) and (len(env.world.obstacles)!=0):
                break
