...
10. no exploration eps
'''


Env = DubinsCarEnv
name_dict = generate_default_model_name(Env)
TXT_NAME = '1model_'+Env.__name__+'_v11.txt'
BMODEL_PATH = name_dict['db'].replace('.pt', '_v11.pt')
...
if __name__ == '__main__':
    ...
    for epoch_i in pbar:
        ...
        if CYCLIC:
            explore_eps = (EXPLORE_EPS - (epoch_i % 100)/50.) if ((epoch_i % 100) < 50) else 0.
        else:
            explore_eps = np.clip(EXPLORE_EPS - 1.5 * (epoch_i // N_VALID) / (N_TRAJ // N_VALID), 0, 0.5)
