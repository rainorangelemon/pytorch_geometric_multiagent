...
if __name__ == '__main__':
    ...
    bnn = HeteroGNN(1024, 3, keys=['agent', 'obstacle'])
    bnn.to(device)
    print(bnn.train())
    ...
    boptimizer = torch.optim.Adam(bnn.parameters(), lr=1e-4)#, momentum=0.9)#, weight_decay=1e-8)
    ...
    BMODEL_PATH = name_dict['db'].replace('.pt', '_fix_start.pt')
    open('1model_start_'+Env.__name__+'.txt', 'w+').close()
    ...
    #     env.world.agents = env.world.sample_agents(env.num_agents, prob=0.0)
        ...
        with open('1model_start_'+Env.__name__+'.txt', 'a+') as f:
