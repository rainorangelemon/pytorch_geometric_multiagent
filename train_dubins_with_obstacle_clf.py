...
def train_barrier(bnn, optimizer, buf, pbar, n_iter=10):
    ...
    for i in range(n_iter):
        for data, next_data in loader:
            ...
            loss = bloss + dloss
            loss.backward()   
            torch.nn.utils.clip_grad_norm_(bnn.parameters(), 0.5)
            optimizer.step()
            optimizer.zero_grad()
...
def train_lyapunov(lnn, optimizer, buf, pbar, n_iter=10):
    ...
    for i in range(n_iter):
        for data, next_data in loader:
            optimizer.zero_grad()
            bloss, dloss = compute_loss(lnn, data.to(device), next_data.to(device))
            loss = bloss + dloss
            loss.backward()   
            torch.nn.utils.clip_grad_norm_(lnn.parameters(), 0.5)
            optimizer.step()
            optimizer.zero_grad()
            pbar.set_description(version_name+": iter #%d, bloss %.6f, dloss %.6f" % (i, bloss, dloss))
            wandb.log({"bloss": bloss,
