    ...
    for i in range(n_iter):
        for data, next_data in loader:
            ...
            if CLIP_NORM is not None:
                torch.nn.utils.clip_grad_norm_(bnn.parameters(), CLIP_NORM)
            optimizer.step()
            optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    for epoch_i in pbar:
        ...
        if (OUTDATE == float('inf')) and (epoch_i % N_VALID==0):
            ...
            while True:
                if (len(cbuf.obs_buf)>0) and (cbuf.obs_buf[0]['epoch_i'] < (epoch_i - OUTDATE)):
                    cbuf.obs_buf.pop(0)
                else:
                    break
        ...
        while True:
            ...
            if done or (total_trans >= max_episode_length):
                ...
                if not CBUF_BEFORE_RELABEL:
                    if RELABEL:
                        if PER_STEP_EXPLORE or RELABEL_IF_EXPLORE or (not explore_now):
